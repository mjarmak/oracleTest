package src.course2.streams.parallelStreams;/*
...
public class ParallelReduction {
    ...
    public static void main(String[] args) {
        ...
        System.out.println("serial sum using (reduce) method reference" +
        ...
        System.out.println("parallel sum using (reduce) method reference" +
                "(Integer::sum) as accumulator = " + sum);
        ...
        System.out.println("parallel sum using (reduce) lambda expression " +
                "((s1, s2) -> s1 + s2) as accumulator = " + sum);

        // Use the reduce combiner terminal operation on a serial stream
        sum = Stream.iterate(0, i -> i <= 10000, i -> i + 1)
                // three args reduce method
                .reduce(1, (s1, s2) -> s1 + s2, (s1, s2) -> 10);
        System.out.println("serial sum using (reduce) lambda expression " +
                "((s1, s2) -> s1 + s2) as accumulator\n\t" +
                "and ((s1,s2) -> s1) as combiner= " + sum);

        // Use the reduce combiner terminal operation on a parallel stream
        sum = Stream.iterate(0, i -> i <= 10000, i -> i + 1)
                .parallel()
                // three args reduce method
